...
order.onclick = function () {
  alert("Qúy khách vui lòng điền thông tin")
...
for (var i = 0; i < add_cart.length; i++) {
  ...
  add.addEventListener("click", function (event) {
    ...
    var img = product.parentElement.getElementsByClassName("anhh1")[0].src
   
    var title = product.getElementsByClassName("box-end")[0].innerText
...
for (var i = 0; i < add_cart.length; i++) {
  ...
  add.addEventListener("click", function (event) {
    ...
    var button = event.target;
    var product = button.parentElement.parentElement;
    var img = product.parentElement.getElementsByClassName("anh1")[0].src
    var title = product.getElementsByClassName("box-end")[0].innerText
    var price = product.getElementsByClassName("gia")[0].innerText
    addItemToCart(title, price, img)
    // Khi thêm sản phẩm vào giỏ hàng thì sẽ hiển thị modal
...
function addItemToCart(title, price, img) {
  ...
  <div class="cart-item cart-column">
      <img class="img" src="${img}" alt=""  height="40px">
      <p>${title}<p>
  </div>
